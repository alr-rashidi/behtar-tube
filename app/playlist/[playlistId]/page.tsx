...
const page = async ({ params }: any) => {
  ...
  const videos = data.videos.filter(video => video.title != '[Private video]')
      ...
      {data ? (
        <div className="flex flex-col px-4">
          <div aria-label="imageBox" className="relative mb-10">
            <Image
              src={videos[0].videoThumbnails[0].url}
              width={videos[0].videoThumbnails[0].width}
              height={videos[0].videoThumbnails[0].height}
          ...
          <div className="mb-8 text-2xl font-extrabold md:text-3xl">{data.title}</div>
          <div className="flex flex-row gap-4">
            ...
            <div className="flex flex-col justify-center gap-2">
              <Link href={`/channel/${data.authorId}`} className="text-lg font-bold underline">{data.author}</Link>
              <div className="text-sm">{numberCounter(data.videoCount)} Videos</div>
            </div>
          </div>
          <div className="flex flex-col gap-4 p-4">
            {videos.map(video => video.title != '[Private video]' ? <VideoListItem key={video.videoId} video={video} /> : null)}
          </div>
        </div>
      ) : (
        <></>
